...
**Current phase: Phase 4 in progress** — `edges` table, conflict detector (`updates` / `extends` / `derives` / `contradicts` / `duplicate` / `new`), and one-hop graph expansion at query time landed. The ingestion pipeline writes edges and supersedes prior memories; `/v1/search` accepts `expand_graph: true` to surface neighbours. New eval categories (`knowledge_update`, `multi_session`) are in tree, with the runner now ingesting `setup_sessions` as separate conversations so the conflict detector sees committed prior memories. Eval deltas against the Phase 4 targets (>75% knowledge_update, >65% multi_session) and the manual TS→Rust regression test from § exit criteria still pending an `OPENAI_API_KEY`. Phase 3 work — contextualizer, hybrid search (vector + tsvector + RRF), reranker, reingest script — remains in place; vector index is still opt-in via `pnpm db:vector-index`.
