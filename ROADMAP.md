...
**Current phase: Phase 5 in progress** — two-axis time landed. Extraction prompt bumped to `extraction_v2` and now resolves relative phrases against `document_date` to populate `event_date` + `event_date_precision` per memory. New `src/retrieval/temporal-parser.ts` (LLM call backed by `temporal_parser_v1.txt`) infers a `{axis, from, to}` window from the query at search time; `src/retrieval/temporal-filter.ts` applies it post-RRF, pre-rerank. `MemCore.search` accepts an explicit `dateRange` (or `null` to disable auto-parse) and `/v1/search` mirrors the SPEC shape via `filters.date_range`. Memories with `event_date IS NULL` are kept under event_date filters (timeless preferences/identity facts shouldn't disappear from a windowed query). New `evals/cases/temporal_reasoning.jsonl` with 12 cases. Phase 4 work — `edges` table, conflict detector, one-hop graph expansion — remains in place; the Phase 4 eval-target deltas (>75% knowledge_update, >65% multi_session) and the new Phase 5 target (>70% temporal_reasoning) still pend an `OPENAI_API_KEY` run.
